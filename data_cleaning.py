...
def load_and_clean_stock_data(
    ...
    """
    Load engineered data and apply cleaning pipeline.
    ...
    --------
    Dict[str, pd.DataFrame]
        Dictionary with cleaned DataFrames for each ticker
    """
    ...
    for ticker in tickers:
        ...
        filepath = f"{data_dir}/{ticker}_features.csv"
        
        try:
            # Read CSV with multi-level column headers
            df = pd.read_csv(filepath, skiprows=[1, 2], index_col=0)
            df.index = pd.to_datetime(df.index, format='%Y-%m-%d')
            
            # Clean up column names if they have extra levels
            ...
            df_cleaned = DataCleaner.clean_dataset(
                df,
...
def display_cleaned_data_summary(cleaned_data: Dict[str, pd.DataFrame]):
    """
    ...
    for ticker, df in cleaned_data.items():
        ...
        # Handle both datetime and string index
...
def display_sample_verification(cleaned_data: Dict[str, pd.DataFrame], n_rows: int = 5):
    """
    ...
    print("\n\n" + "="*80)
    print(f"VERIFICATION - Last {n_rows} Rows (No NaN Values)")
    print("="*80)
    
    for ticker, df in cleaned_data.items():
        print(f"\n{ticker}")
        print("-" * 80)
        print(df.tail(n_rows).round(4))
        print(f"\nNaN count in last {n_rows} rows: {df.tail(n_rows).isna().sum().sum()}")
...
def save_cleaned_data(
    ...
    for ticker, df in cleaned_data.items():
        filepath = f"{output_dir}/{ticker}{suffix}.csv"
        df.to_csv(filepath)
...
def main():
    ...
    cleaned_data = load_and_clean_stock_data(
        tickers=tickers,
        data_dir='data',
        remove_nan=True,
        verbose=True
    ...
    # Verify cleaned data
    display_sample_verification(cleaned_data, n_rows=5)
    
    # Save cleaned datasets
    save_cleaned_data(cleaned_data, output_dir='data', suffix='_cleaned')
